...
export const MultiSceneDarkTemplate: React.FC<{ scene: any }> = ({ scene }) => {
    ...
    return (
        <AbsoluteFill style={{ backgroundColor: '#000', color: 'white', fontFamily: '"Inter", sans-serif' }}>
            ...
            {layoutSkin === 'podcast_wave' && <SinglePodcastWave content={scene.visual_content} scene={scene} />}
            {layoutSkin === 'split_compare' && <SingleSplitCompare content={scene.visual_content} />}
